...
class EditRecipe extends React.Component{
    ...
    componentDidMount(){
        this.textarea.focus();
        autosize(this.textarea);
        $("editRecipeModal" + this.props.recipe.id).on('hidden.bs.modal', function () {
            $(this).find("input,textarea,select").val([]).end();
        });
    }
    ...
    updateRecipe(recipe) {
        axios.put('/recipes/' + recipe.id, {
            ...
            name: recipe.name,
            steps: recipe.steps,
            ingredients: recipe.ingredients}).then(response => {
            client({method: 'GET', path: '/recipes'}).done(response => {
    ...
    render() {
        return (
            <React.Fragment>
                <div class="modal fade" id={"editRecipeModal" + this.props.recipe.id} tabindex="-1" role="dialog">
                    <div class="modal-dialog modal-xl" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Edit recipe: <i>{this.props.recipe.name}</i></h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                        <div class="modal-body">
                            <div class="container-fluid">
                                <form>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <div class="md-form">
                                                <input type="text" id="name-form" value={this.state.name} class="form-control" name="name" onChange={event => this.handleNameChange(event)}/>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal" onClick={event => this.handleClose(event)}>Close</button>
                                <button type="button" class="btn btn-primary" onClick={event => this.handleSubmit(event)}>Save recipe</button>
                            </div>
                        </div>
                    </div>
                </div>
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target={"#editRecipeModal" + this.props.recipe.id}>
                    Edit recipe
                </button>
            </React.Fragment>
        )
            ...
            <React.Fragment>
                <div class="modal fade" id={"showDetailsModal" + this.props.recipe.id} tabindex="-1" role="dialog">
                  <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title"><i>{this.props.recipe.name}</i></h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                          <span aria-hidden="true">&times;</span>
                        </button>
                      </div>
	...
	render() {
		return (
			<tr>
				...
				{' '}
				<EditRecipe recipe={this.props.recipe} update={this.props.update}/>
				</td>
			</tr>
		)
	}
